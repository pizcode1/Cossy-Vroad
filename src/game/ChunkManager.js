...
export class ChunkManager {
    ...
    rows = new Map();
    maxGenerated = -1;
    laneIndices = Array.from({ length: LANE_WIDTH }, (_, i) => i - Math.floor(LANE_WIDTH / 2));
    constructor(scene) {
        this.scene = scene;
    ...
    ensureRows(playerRow, difficulty, character) {
    ...
    createRow(index, character, difficulty) {
        ...
        const floor = new THREE.Mesh(floorGeom, new THREE.MeshLambertMaterial({ color: baseColor }));
        ...
        if (rowType === 'grass' && !safe) {
            ...
            for (let i = 0; i < treeCount; i += 1) {
                ...
                const trunk = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.35, 0.25), new THREE.MeshLambertMaterial({ color: 0x835b2a }));
                const crown = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.6, 0.7), new THREE.MeshLambertMaterial({ color: character.biome === 'snow' ? 0xd9f5ff : 0x2e9d46 }));
    ...
    pickRowType(index, difficulty) {
        ...
        if (roll < railWeight)
            return 'rail';
        if (roll < railWeight + waterWeight)
            return 'water';
        if (roll < railWeight + waterWeight + roadWeight)
            return 'road';
        return 'grass';
    }
    emptyOccupancy() {
